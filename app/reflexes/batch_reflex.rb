class BatchReflex < ApplicationReflex
  ...
  def show
    ...
    if contacts.size > 0
      ...
      profile = render(partial: "batches/profile", locals: { contact: "", group_id: batch.id })
    end
    morph "#show1", "#{html}"
  ...
  def contact
    ...
    html = render(partial: "batches/profile", locals: { contact: contact, event: @event, call: @call, group_id: batch.id })
    morph "#profile", "#{html}"
  end

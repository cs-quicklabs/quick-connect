class SubscriptionManager
  ...
  def template
    ...
    if payment_processor.subscribed? and payment_processor.subscription.active?
      template = "manage"
      return template
    end

    # user has a stripe processor and subscription which was cancelled but is still under grace period
    if payment_processor.subscribed? and payment_processor.subscription.cancelled? and payment_processor.subscription.ends_at > Time.zone.now
      template = "renew"
      return template
    end

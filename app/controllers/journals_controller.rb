class JournalsController < BaseController
  ...
  def index
    authorize Journal
    @rating = Rating.find_by_date(Date.today)
    @ratings_by_month = Rating.select("DISTINCT ON (date)*").all.where("date <= ? and date > ?", Date.today, Date.today - 6.months).order(date: :desc).group_by { |r| r.date.beginning_of_month }

class Profile::CallsController < Profile::BaseController
    before_action :set_call, only: %i[ show edit update destroy] 
      ...
      def update
        ...
        respond_to do |format|
          if @call.update(call_params)
            format.turbo_stream { render turbo_stream: turbo_stream.replace(@call, partial: "profile/calls/call", locals: { call: @call }) }
          else
            format.turbo_stream { render turbo_stream: turbo_stream.replace(@call, template: "profile/calls/edit", locals: { call: @call }) }
          end
        end
      end
      ...
      def create
        ...
        respond_to do |format|
          if @call.persisted?
            format.turbo_stream {
              render turbo_stream: turbo_stream.prepend(:calls, partial: "profile/calls/call", locals: { call: @call }) +
                                   turbo_stream.replace(Call.new, partial: "profile/calls/form", locals: { call: Call.new })
            }
          else

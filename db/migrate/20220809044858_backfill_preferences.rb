class BackfillPreferences < ActiveRecord::Migration[7.0]
  def change
    ...
    accounts.each do |account|
      ...
      Preference.new(key: "transfer_data_to_admin", value: User.where(permission: :admin).first.id, title: "Transfer data to admin on user delete", message: "When a user is deleted, you might want to keep some of the data like nuggets, Knowledge Base or Report Templates. Whom do you wish to assign this data when the user is deleted").save
    end
  end
end
